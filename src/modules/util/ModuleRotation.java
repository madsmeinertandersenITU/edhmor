...
public class ModuleRotation {
     ...
     *
     */
    public ModuleRotation(int type, int orientation, Vector3D parentFaceNormal, Vector3D parentFaceCoplanar) {
        ...
        //System.out.println("parentFaceNormal (negated): " + parentFaceNormal.toString());
        ...
        //System.out.println("childFaceNormal: " + childFaceNormal.toString());
        
        //get the coplanar vector of the child face where it will be connected 
        Vector3D childFaceCoplanar = moduleSet.getCoplanarFaceVector(type, faceToConnect);
        //System.out.println("childFaceCoplanar: " + childFaceCoplanar.toString());

        //Obtain a rotation matrix thar rotates the module to align the 
        //child face with the face of the parent
        Rotation firstRotation;
        if(parentFaceCoplanar!=null && childFaceCoplanar!=null){
            //Completely defined rotation, works always well
            firstRotation = new Rotation(childFaceNormal, childFaceCoplanar, parentFaceNormal, parentFaceCoplanar);
        }else{
            //This align the faces but the rotation along the normal vector is not defined
            //Works well for modules with orthogonal connectors, but it does not work for Atron modules 
            firstRotation = new Rotation(childFaceNormal, parentFaceNormal);
        }
        //System.out.println("firstRotation: ");
        new ModuleRotation(firstRotation).getEulerAngles();
        ...
        //System.out.println("rotationAboutNormal: ");
        new ModuleRotation(rotationAboutNormal).getEulerAngles();
...
//        System.out.println("Euler Angles: " + angles[0]/Math.PI*180.0+ " "+ angles[1]/Math.PI*180.0 + " " + angles[2]/Math.PI*180.0);
    ...
    /***
     * Depreciated. Calculates the Euler Angles using the library and handles 

 ...
 * @author fai
 */
public class RobotBestFeaturesLogTool extends ParallelLogTool{
    
    @Override
    public String getLogID() {
        return "ParallelRobotBestFeaturesLogTool";
    }
    ...
    public void update(Observable o, Object arg) {
        ...
        if (pea.getCurrentObservable() instanceof EvolutionaryAlgorithm) {
            ...
            if (algorithm.getState() == EvolutionaryAlgorithm.REPLACE_STATE && arg == null) {
                ...
                String typePercStr = "";
                double[] typePercentage = features.getTypePercentage();
                ...
                super.getLog().println(
                        ...
                        + typePercStr
                        + features.getRobotMass() + " - "
                        + best.getObjectives().get(RoboticObjetiveFunction.OBJECTIVE_BROCKEN_CONN_WORLD_1) + " - "
                        + features.getDimensions().x + " - "
                        + features.getDimensions().y + " - "
                        + features.getDimensions().z + " - "
                        + features.getInertia().getX() + " - "
                        + features.getInertia().getY() + " - "
                        + features.getInertia().getZ() + " - "
                        + features.getAverageConnectionsPerModule() + " - "
                        + features.getDispersionConnectionsPerModule() + " - "
                        );
            }
        }
    }
